...
It uses the [imdbinfo](https://github.com/tveronesi/imdbinfo) package to fetch movie details based on the IMDB ID or
title and is powered by [FastAPI](https://fastapi.tiangolo.com/).
...
5. The API exposes the following endpoints:
   ...
   - `/awards/<imdb_id>[?locale=<locale>]`
      - example: `http://127.0.0.1:5000/awards/tt0944947?locale=en-US`
      - returns the awards information for a movie or series
...
For details on how to use the `imdbinfo` package, please refer to the [imdbinfo](https://github.com/tveronesi/imdbinfo)
GitHub repository.
...
This project and its authors are not affiliated in any way to Internet Movie Database Inc.; see the [
`DISCLAIMER.txt`](DISCLAIMER.txt) file for details about data licenses.
